...
class pm_debit extends PaymentRoot {
    ...
    function showPaymentForm($params, $pmconfigs) {
        if (!isset($params['acc_holder']))
            $params['acc_holder'] = '';
        if (!isset($params['bank_iban']))
            $params['bank_iban'] = '';
        if (!isset($params['bank_bic']))
            $params['bank_bic'] = '';
    ...
    function showAdminFormParams($params) {
        JSFactory::loadExtLanguageFile('pm_debit_bic_required');
        $array_params = array('bic_required');
