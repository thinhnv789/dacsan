...
if (!class_exists('modJUXMegaMenu')) {
    require_once dirname(__FILE__) . '/includes/css.php';
}
class mod_JUX_MegaMenuHelper {
    public static function getCssProcessor(&$params, $filename, $prefix) {
        return modJUXMegaMenu::process($params, $filename, $prefix);
    }
...
//				$txt = '<span class="menu-image no-title"><img src="'.$item->menu_image.'" alt="'.$tmpname.'" /></span>';
			...
			$txt = '<span class="menu-title"><i class="jux-fa  jux-'.$font_awesome_icon.'"></i>' . $tmpname . '</span>';
		}
	...
	function beginMenuItems($pid=0, $level=0, $return=false){
		if ($level) {
			if ($this->items[$pid]->megaparams->get('group')) {
				...
				$style = $this->getParam ('megacss-style', 1);
...
//		if (!preg_match ('/group/', $cls)) $cls .= ($active?" active":"");
	...
	function beginMenuItems1($pid = 0, $level = 0, $return = false) {
        // $custtom_css=$param->get('mega_style');

        $cols = $pid && $this->getParam('megamenu') && isset($this->items[$pid]->cols) && $this->items[$pid]->cols ? $this->items[$pid]->cols : 1;
        $width = $this->items[$pid]->megaparams->get('width', 0);
        //$t_s	= $tmp->params->get('mega_style', '');
        $custtom_css = $this->items[$pid]->params->get('mega_style', 0);
//                var_dump($this->items[$pid]->params);die;
        if (!$width) {
            for ($col = 0; $col < $cols; $col++) {
        ...
        $data = "<div style='$custtom_css' class=\"childcontent adddropdown clearfix cols$cols $right \">\n";
        $data .= "<div class=\" jux-fa jux-fa-angle-down dropdown-toggle \" id=\"arrow-icon\" data-toggle=\"dropdown\"></div>\n";
        $data .= "<div class=\"childcontent-inner-wrap dropdown-menu\">\n";  //Add wrapper
