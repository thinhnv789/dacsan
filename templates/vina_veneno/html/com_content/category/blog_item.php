    ...
    <section class="entry-content media">  
		...
		<div class="article-text">
			...
			<?php 
				...
				?>
				<?php 
					if ((int) $this->columns == 1) {
						$create_date_on = 'VINA_CREATED';
						$published_date_on = 'VINA_PUBLISHED';
						$written_by_text = 'VINA_WRITTEN_BY';
						$content_parent = 'VINA_CONTENT_PARENT';
						$content_category = 'VINA_CONTENT_CATEGORY';
					}
					else {
						$create_date_on = '';
						$published_date_on = '';
				...
				?>
				<div class="entry-meta article-attr muted clearfix">
					<?php /*------------------------ Date Block ------------------------*/ ?>
					<?php // Create Date ?>
					<?php if ($params->get('show_create_date')) : ?>
						<time class="create-date" datetime="<?php echo $this->item->created; ?>">
							<i class="icon-time"></i>
							<?php 
								echo JText::_($create_date_on);
								print JHtml::_('date',$this->item->created, JText::_('DATE_FORMAT_LC')); 
							?>
						</time>
					<?php endif; ?>
					<?php // Publish Date ?>
					<?php if ($params->get('show_publish_date')) : ?>
						<time class="publish-date" datetime="<?php echo $this->item->publish_up; ?>">
							<?php 
								echo JText::_($published_date_on);
					...
					<?php if ($params->get('show_author') && !empty($this->item->author )) : ?>
						...
						<span class="by-author"> 
							<?php 
								...
								echo JText::_($written_by_text);
								echo $written_by;
							?>
						</span>
					<?php endif; ?>		
					
					<?php /*------------------------ Parent Category Block ------------------------*/ ?>
					...
					<?php if ($params->get('show_parent_category') && !empty($parentslug) && $this->item->parent_id !=1) : ?>
						...
						<span class="parent-category-name">
							<?php 
								...
								if ($params->get('link_parent_category') and !empty($parentslug)) :
									echo JText::_($content_parent);
									echo $url;
								else :
									echo JText::_($content_parent);
									echo $title;
					...
					<?php if ($params->get('show_category')) : ?>
						...
						<span class="category-name">
							<?php 
								...
								if ($params->get('link_category') and $catslug) :
									echo JText::_($content_category);
									echo $url;
								else :
									echo JText::_($content_category);
									echo $title;
