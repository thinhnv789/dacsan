...
if (count($rows) > 0) {
	$i = 0;
	$row_product = 3;	
	$row_number = count($rows)/$row_product;
							...
							<div class="price_block">
								<?php if( $product_old_price){?>
									<?php if ($product->product_old_price > 0){// option modul product_old_price?>
										<p class="old_price"><?php if ($jshopConfig->product_list_show_price_description) print _JSHOP_OLD_PRICE.": ";?><span><?php print formatprice($product->product_old_price)?></span></p>
									<?php }?>
								...
								<?php if ($product->product_price_default > 0 && $jshopConfig->product_list_show_price_default && $product_price_default){ // option modul product_price_default?>
									<p class="default_price"><?php print _JSHOP_DEFAULT_PRICE.": ";?><span><?php print formatprice($product->product_price_default)?></span></p>
								<?php }?>
							</div>
						<?php }?>
						<!-- End Price and Old Price -->
						...
						<?php print $product->_tmp_var_top_buttons;?>
							<div class="buttons vina_bottons">																					
								<?php if ($product->buy_link && $show_button_buy){?>
									<a class="button_buy" title="<?php print JText::_('JSHOP_ADD_TO_CART'); ?>" href="<?php print $product->buy_link?>"><i style="display: inline-block;" class="icon-shopping-cart"></i><?php //print JText::_('JSHOP_ADD_TO_CART'); ?></a>
								<?php }?>
								<?php if ($show_button_detal){?>
									<a class="button_detail" title="<?php print JText::_('JSHOP_VIEW_DETAIL'); ?>" href="<?php print $product->product_link?>"><i style="display: inline-block;" class="icon-eye-open"></i><?php //print JText::_('JSHOP_VIEW_DETAIL'); ?></a>
								<?php }?>
								<a class="button_wishlist" title="<?php print JText::_('JSHOP_ADD_TO_WISHLIST'); ?>" href = "<?php print SEFLink('index.php?option=com_jshopping&controller=cart&task=add&category_id='.$product->category_id.'&product_id='.$product->product_id.'&to=wishlist', 1);?>"><i style="display: inline-block;" class="icon-heart"></i><?php //print JText::_('JSHOP_ADD_TO_WISHLIST'); ?></a>
