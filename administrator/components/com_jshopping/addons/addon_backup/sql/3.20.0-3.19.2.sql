...
ALTER TABLE `#__jshopping_shipping_method`
  ...
  DROP INDEX ordering,
  DROP INDEX published,
  CHANGE COLUMN `name_en-GB` `name_en-GB` VARCHAR(100) NOT NULL AFTER shipping_id,
  CHANGE COLUMN `description_en-GB` `description_en-GB` TEXT NOT NULL AFTER `name_en-GB`,
  CHANGE COLUMN `name_de-DE` `name_de-DE` VARCHAR(100) NOT NULL AFTER `description_en-GB`,
  CHANGE COLUMN `description_de-DE` `description_de-DE` TEXT NOT NULL AFTER `name_de-DE`,
  CHANGE COLUMN published published TINYINT(1) NOT NULL AFTER `description_de-DE`,
  CHANGE COLUMN payments payments VARCHAR(255) NOT NULL AFTER published,
  CHANGE COLUMN image image VARCHAR(255) NOT NULL AFTER payments,
  CHANGE COLUMN ordering ordering INT(6) NOT NULL AFTER image,
  CHANGE COLUMN alias alias VARCHAR(100) NOT NULL AFTER ordering;
